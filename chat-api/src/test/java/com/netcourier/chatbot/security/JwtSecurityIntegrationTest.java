...
class JwtSecurityIntegrationTest {
    ...
    @Test
    void rejectsJsonIngestionWithoutJwt() {
        webTestClient.post()
                .uri("/api/ingest")
    ...
    @Test
    void allowsJsonIngestionWithJwt() {
        Mockito.when(ingestionService.ingestText(any()))
                .thenReturn(sampleResponse());

        webTestClient.mutateWith(SecurityMockServerConfigurers.mockJwt(jwt -> jwt.claim("roles", List.of("CP"))))
                .post()
                .uri("/api/ingest")
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(sampleTextRequest())
    ...
    @Test
    void rejectsAdminIngestionWithoutJwt() {
        webTestClient.post()
    ...
    @Test
    void allowsAdminIngestionWithJwt() {
        Mockito.when(ingestionService.ingestDocument(any()))
                .thenReturn(sampleResponse());

        webTestClient.mutateWith(SecurityMockServerConfigurers.mockJwt(jwt -> jwt.claim("roles", List.of("ADMIN"))))
                .post()
                .uri("/admin/ingest/upload")
                .contentType(MediaType.MULTIPART_FORM_DATA)
                .body(BodyInserters.fromMultipartData(sampleMultipart()))
